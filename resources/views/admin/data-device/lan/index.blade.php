...
@section('content')
    <div class="section-body">
        <h1 class="mb-3">Data Device LAN (LAN)</h1>
        <div class="card">
            <div class="card-body">
                <form action="/data-lan" method="POST">
                    @csrf
                    <div class="mb-3">
                        <label class="form-label">Kode Device</label>
                        <input type="text" name="kode_device" class="form-control" placeholder="Contoh: LAN-1" required>
                    </div>
                    @error('kode_device')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="nama" class="form-control" placeholder="Contoh: LAN 1" required>
                    </div>
                    @error('nama')
                        <div class="alert alert-danger">{{ $message }}</div>
                    @enderror
                    <div class="mb-3">
                        ...
                        <input type="text" name="merk" class="form-control" placeholder="Contoh: DIGILINK" required>
                    </div>
                    @error('merk')
                        <div class="alert alert-danger">{{ $message }}</div>
                    @enderror
                        ...
                        <tbody>
                            @forelse ($datalan as $key => $value)
                                <tr>
                                    ...
                                    <td>{{ $value->status }}</td>
                                    <td>
                                        <form action="/data-lan/{{ $value->id }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <a href="/data-lan/{{ $value->id }}/edit" class="btn"
                                                style="background-color:#14274c; color:white">Edit</a>
                                            <a href="/data-lan/{{ $value->id }}/delete" class="btn btn-danger"
