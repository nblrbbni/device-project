            ...
            <div class="card-body">
                <form action="/data-computer" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="nama" class="form-control" placeholder="Contoh: PC 1 (LAB 1)"
                            required>
                    </div>
                    @error('nama')
                        <div class="alert alert-danger">{{ $message }}</div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="merk" class="form-control" placeholder="Contoh: PC 1 (LAB 1)"
                            required>
                    </div>
                    @error('merk')
                        <div class="alert alert-danger">{{ $message }}</div>
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-bordered table-hover text-center" id="myTable">
                        ...
                        <tbody>
                            @forelse ($datacomputer as $key => $value)
                                <tr>
                                    <td>{{ $key + 1 }}</td>
                                    ...
                                    <td>{{ $value->status }}</td>
                                    <td>
                                        <form action="/data-computer/{{ $value->id }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <a href="/data-computer/{{ $value->id }}/edit" class="btn"
                                                style="background-color:#14274c; color:white">Edit</a>
                                            <a href="/data-computer/{{ $value->id }}/delete" class="btn btn-danger"
