...
@section('content')
    <div class="section-body">
        <h1 class="mb-3">Data Device Tablet (TAB)</h1>
        <div class="card">
            <div class="card-body">
                <form action="/data-tablet" method="POST">
                    @csrf
                    <div class="mb-3">
                        <label class="form-label">Kode Device</label>
                        <input type="text" name="kode_device" class="form-control" placeholder="Contoh: TAB-1" required>
                    </div>
                    @error('kode_device')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="nama" class="form-control" placeholder="Contoh: TABLET 1" required>
                    </div>
                    @error('nama')
                        <div class="alert alert-danger">{{ $message }}</div>
                    @enderror
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-bordered table-hover text-center" id="myTable">
                        ...
                        <tbody>
                            @forelse ($datatablet as $key => $value)
                                <tr>
                                    <td>{{ $key + 1 }}</td>
                                    ...
                                    <td>{{ $value->status }}</td>
                                    <td>
                                        <form action="/data-tablet/{{ $value->id }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <a href="/data-tablet/{{ $value->id }}/edit" class="btn"
                                                style="background-color:#14274c; color:white">Edit</a>
                                            <a href="/data-tablet/{{ $value->id }}/delete" class="btn btn-danger"
