...
@section('content')
    <div class="section-body">
        <h1 class="mb-3">Data Pengembalian Device Computer</h1>
        <div class="card">
            <div class="card-header">
                <h4>Pengembalian Computer</h4>
            </div>
            <div class="card-body">
                    ...
                    <table class="table table-bordered table-hover text-center" id="myTable">
                        <thead>
                            <tr>
                                ...
                                <th class="text-center">Kondisi PC</th>
                        ...
                        <tbody>
                            @forelse ($_p_c_penembalian as $key => $value)
                                <tr>
                                    <td class="text-center">{{ $key + 1 }}</td>
                                    <td>{{ \Carbon\Carbon::parse($value->tanggal_peminjaman)->format('d-m-Y') }}</td>
                                    <td>{{ $value->nama }}</td>
                                    <td>{{ $value->kode_device }}</td>
                                    <td>{{ $value->kondisi_pc }}</td>
                                    <td>{{ $value->kondisi_mouse }}</td>
                                    <td>{{ $value->kondisi_keybohard  }}</td>
                                    <td>{{ $value->waktu_pengembalian }}</td>
                                    <td>
                                        <form action="/return-computer/{{ $value->id }}/delete" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <a href="/return-computer/{{ $value->id }}/delete" class="btn btn-danger" onclick="confirmation(event)"
                                                data-id="{{ $value->id }}"
