...
@section('content')
    <div class="section-body">
        <h1 class="mb-3">Data Pengembalian Device HDMI</h1>
        <div class="card">
            <div class="card-header">
                <h4>Pengembalian HDMI</h4>
            </div>
            <div class="card-body">
                    ...
                    <table class="table table-bordered table-hover text-center" id="myTable">
                        <thead>
                            <tr>
                                ...
                                <th class="text-center">Kondisi HDMI</th>
                                <th class="text-center">Waktu Pengembalian</th>
                                <th class="text-center">Action</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse ($_h_d_m_i_penembalian as $key => $value)
                                <tr>
                                    <td class="text-center">{{ $key + 1 }}</td>
                                    <td>{{ \Carbon\Carbon::parse($value->tanggal_peminjaman)->format('d-m-Y') }}</td>
                                    <td>{{ $value->nama }}</td>
                                    <td>{{ $value->kode_device }}</td>
                                    <td>{{ $value->kondisi_HDMI }}</td>
                                    <td>{{ $value->waktu_pengembalian }}</td>
                                    <td>
                                        <form action="/return-HDMI/{{ $value->id }}/delete" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <a href="/return-HDMI/{{ $value->id }}/delete" class="btn btn-danger" onclick="confirmation(event)"
                                                data-id="{{ $value->id }}"
